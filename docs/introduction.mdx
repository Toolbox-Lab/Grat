...
**Grat** turns opaque Soroban errors into plain English, replays historical transactions against reconstructed ledger state, and lets you step through contract execution with full time-travel debugging. 
...
Today, developers debug these errors by grepping through Stellar Core source code, adding print statements, or asking on Discord. Grat is the missing diagnostic layer designed to give you the answer in seconds, not hours.

## The 3-Tier Model

Grat is organized into three tiers of depth, each building on the one before it.
