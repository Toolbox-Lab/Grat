...
<Steps>
  ...
  <Step title="Run Decode">
    Run `grat decode <hash>`. Grat identifies that the `InvalidInput` happened inside the `Token` contract during a transfer call.
  </Step>
  <Step title="Esclate to Trace">
    Run `grat trace <hash>`. The hierarchical timeline shows that the `swap` contract passed a negative amount to the `token` contract.
  </Step>
